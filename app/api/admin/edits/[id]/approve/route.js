...
import { NextResponse } from 'next/server'
  ...
  const DIRECTORY_LISTING_FIELDS = ['businessName', 'location', 'telephone', 'website']
  ...
  export async function PATCH(req, { params }) {
    try {
      ...
      if (type === 'directory_listing_edit') {
        ...
        const directoryListingData = Object.fromEntries(
          Object.entries(edit.changes || {}).filter(([field]) => DIRECTORY_LISTING_FIELDS.includes(field))
        )
