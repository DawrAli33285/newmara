...
export async function PATCH(req, { params }) {
  try {
    ...
    if (billingInterval != null && !validIntervals.includes(billingInterval)) {
      return NextResponse.json(
        { error: `billingInterval must be one of: ${validIntervals.join(', ')}.` },
        { status: 400 }
      )
    }
    ...
    const printPackage = await prisma.printPackage.update({
      ...
      data: {
        ...
        ...(billingInterval !== undefined && { billingInterval: billingInterval || null }),
        ...(entitlements !== undefined && { entitlements }),
        ...(isActive !== undefined && { isActive }),
      },
    })
