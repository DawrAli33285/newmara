"use client";

import { useState, useEffect, useCallback, useMemo } from "react";

const SYNC_STATUS_LABELS = {
  success: "Synced",
  pending: "Pending",
  failed: "Failed",
};

const SYNC_STATUS_STYLES = {
  success: "bg-[#edf5df] text-[#527323]",
  pending: "bg-amber-50 text-amber-600",
  failed: "bg-red-50 text-red-600",
};

const EVENT_LABELS = {
  new_subscriber: "New subscriber",
  new_advertiser: "New advertiser",
  new_digital_partner: "New Digital Partner",
  package_selected: "Package selected",
  subscription_activated: "Subscription activated",
  subscription_cancelled: "Subscription cancelled",
  subscriber_renewal: "Subscriber renewal approaching",
  advertiser_renewal: "Advertiser renewal approaching",
  offer_submitted: "Offer submitted",
  lead_generated: "Enquiry / lead generated",
};



function Toast({ toasts, removeToast }) {
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-2">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`flex items-center gap-3 rounded-xl px-4 py-3 shadow-lg text-sm font-semibold text-white transition-all
            ${t.type === "success" ? "bg-[#527323]" : ""}
            ${t.type === "error"   ? "bg-red-600"   : ""}
            ${t.type === "info"    ? "bg-[#0b1830]" : ""}
          `}
        >
          <span>{t.message}</span>
          <button onClick={() => removeToast(t.id)} className="ml-1 opacity-70 hover:opacity-100">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-3.5 w-3.5">
              <path d="M18 6L6 18M6 6l12 12" />
            </svg>
          </button>
        </div>
      ))}
    </div>
  );
}

function useToast() {
  const [toasts, setToasts] = useState([]);
  function addToast(message, type = "success") {
    const id = Date.now();
    setToasts((prev) => [...prev, { id, message, type }]);
    setTimeout(() => removeToast(id), 3500);
  }
  function removeToast(id) {
    setToasts((prev) => prev.filter((t) => t.id !== id));
  }
  return { toasts, addToast, removeToast };
}



function LogDrawer({ log, onClose, onRetry, retrying }) {
  if (!log) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/40">
      <div className="h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between">
          <div>
            <span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest ${SYNC_STATUS_STYLES[log.status]}`}>
              {SYNC_STATUS_LABELS[log.status] || log.status}
            </span>
            <h3 className="mt-2 text-lg font-bold text-[#0b1830]">
              {EVENT_LABELS[log.eventType] || log.eventType}
            </h3>
          </div>
          <button onClick={onClose} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-gray-400 hover:bg-gray-50 hover:text-gray-600">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-4 w-4">
              <path d="M18 6L6 18M6 6l12 12" />
            </svg>
          </button>
        </div>

        <div className="mt-6 space-y-4 text-sm">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Record</p>
            <p className="mt-1 text-[#0b1830]">{log.recordLabel || "—"}</p>
          </div>

          <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">IDs</p>
            <div className="mt-2 space-y-1.5 text-xs text-gray-600">
              <p>
                Mara record:{" "}
                <code className="rounded bg-white px-1.5 py-0.5 text-[11px] text-gray-500">
                  {log.maraRecordId || "—"}
                </code>
              </p>
              <p>
                GHL contact:{" "}
                <code className="rounded bg-white px-1.5 py-0.5 text-[11px] text-gray-500">
                  {log.ghlContactId || "not synced"}
                </code>
              </p>
            </div>
          </div>

          {log.errorMessage && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Error</p>
              <p className="mt-1 whitespace-pre-wrap leading-6 text-red-600">{log.errorMessage}</p>
            </div>
          )}

          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Attempted</p>
            <p className="mt-1 text-[#0b1830]">{new Date(log.createdAt).toLocaleString("en-IE")}</p>
          </div>
        </div>

        {log.status === "failed" && (
          <button
            onClick={() => onRetry(log)}
            disabled={retrying}
            className="mt-6 w-full rounded-xl bg-[#668b2f] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#527323] disabled:opacity-60"
          >
            {retrying ? "Retrying…" : "Retry sync"}
          </button>
        )}
      </div>
    </div>
  );
}



function LogRow({ log, onOpen }) {
  return (
    <button
      onClick={() => onOpen(log)}
      className="flex w-full items-center justify-between gap-4 border-b border-gray-100 px-4 py-3 text-left hover:bg-gray-50"
    >
      <div className="flex min-w-0 items-center gap-3">
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest ${SYNC_STATUS_STYLES[log.status]}`}>
          {SYNC_STATUS_LABELS[log.status] || log.status}
        </span>

        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-[#0b1830]">
            {EVENT_LABELS[log.eventType] || log.eventType}
          </p>
          <p className="truncate text-xs text-gray-400">{log.recordLabel || "—"}</p>
        </div>
      </div>

      <span className="shrink-0 text-xs text-gray-400">
        {new Date(log.createdAt).toLocaleDateString("en-IE", { day: "2-digit", month: "short" })}
      </span>
    </button>
  );
}



function AuditRow({ entry }) {
  return (
    <div className="border-b border-gray-100 px-4 py-3">
      <p className="text-sm text-[#0b1830]">
        <span className="font-semibold">{entry.actor}</span> {entry.action}{" "}
        <span className="font-semibold">{entry.recordLabel}</span>
      </p>
      <p className="mt-0.5 text-xs text-gray-400">
        {new Date(entry.createdAt).toLocaleString("en-IE")}
      </p>
    </div>
  );
}



export default function GhlSyncPage() {
  const [tab,          setTab]          = useState("sync"); 
  const [logs,          setLogs]        = useState([]);
  const [auditEntries,  setAuditEntries] = useState([]);
  const [loading,       setLoading]     = useState(true);
  const [statusFilter,  setStatusFilter] = useState("");
  const [search,        setSearch]      = useState("");
  const [selected,      setSelected]    = useState(null);
  const [retrying,      setRetrying]    = useState(false);

  const [auditPage,       setAuditPage]       = useState(1);
  const [auditTotalPages, setAuditTotalPages] = useState(1);
  const [auditTotalCount, setAuditTotalCount] = useState(0);
  const auditPageSize = 10;

  const { toasts, addToast, removeToast } = useToast();
  const loadData = useCallback(async () => {
    setLoading(true);

    const auditParams = new URLSearchParams();
    auditParams.set("page", auditPage);
    auditParams.set("pageSize", auditPageSize);

    const [syncRes, auditRes] = await Promise.all([
      fetch("/api/admin/ghl-sync-logs"),
      fetch(`/api/admin/audit-log?${auditParams.toString()}`),
    ]);
    const syncData = await syncRes.json();
    const auditData = await auditRes.json();
    setLogs(syncData.logs || []);
    setAuditEntries(auditData.entries || []);
    setAuditTotalPages(auditData.totalPages || 1);
    setAuditTotalCount(auditData.totalCount || 0);
    setLoading(false);
  }, [auditPage]);

  useEffect(() => {
    loadData();
  }, [loadData]);
  const filtered = useMemo(() => {
    return logs.filter((l) => {
      if (statusFilter && l.status !== statusFilter) return false;
      if (search) {
        const q = search.toLowerCase();
        const haystack = `${EVENT_LABELS[l.eventType] || l.eventType} ${l.recordLabel || ""}`.toLowerCase();
        if (!haystack.includes(q)) return false;
      }
      return true;
    });
  }, [logs, statusFilter, search]);

  const failedCount = useMemo(() => logs.filter((l) => l.status === "failed").length, [logs]);

  async function handleRetry(log) {
    setRetrying(true);
    const res = await fetch(`/api/admin/ghl-sync-logs/${log.id}/retry`, { method: "POST" });
    const data = await res.json();
    setRetrying(false);

    if (!res.ok) {
      addToast(data.error || "Retry failed", "error");
      return;
    }

    addToast("Resynced with GoHighLevel", "success");
    setSelected(data.log);
    loadData();
  }

  return (
    <main className="min-h-screen bg-[#f7f8fa] p-6 md:p-8">
      <div className="mx-auto max-w-6xl">

    
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-[#0b1830]">GHL Sync & Audit Trail</h1>
            <p className="mt-2 text-sm leading-6 text-gray-500">
              Data sync status with GoHighLevel, and a history of who changed what.
            </p>
          </div>

          {failedCount > 0 && tab === "sync" && (
            <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-red-50 px-3 py-1.5 text-xs font-bold text-red-600">
              {failedCount} failed sync{failedCount === 1 ? "" : "s"}
            </span>
          )}
        </div>

     
        <div className="mb-5 flex gap-2 border-b border-gray-200">
          {[
            { key: "sync", label: "Sync Log" },
            { key: "audit", label: "Audit Trail" },
          ].map((t) => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={`border-b-2 px-3 pb-2.5 text-sm font-semibold ${
                tab === t.key
                  ? "border-[#668b2f] text-[#0b1830]"
                  : "border-transparent text-gray-400 hover:text-gray-600"
              }`}
            >
              {t.label}
            </button>
          ))}
        </div>

        {tab === "sync" && (
          <>
            {logs.length > 0 && (
              <div className="mb-5 flex flex-wrap items-center gap-3">
                <input
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search event, record…"
                  className="h-10 w-56 rounded-xl border border-gray-200 bg-white px-3 text-sm focus:border-[#668b2f] focus:outline-none"
                />

                <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm focus:border-[#668b2f] focus:outline-none"
                >
                  <option value="">All statuses</option>
                  {Object.entries(SYNC_STATUS_LABELS).map(([value, label]) => (
                    <option key={value} value={value}>{label}</option>
                  ))}
                </select>
              </div>
            )}

            {loading ? (
              <div className="grid place-items-center py-20">
                <p className="text-sm text-gray-400">Loading…</p>
              </div>
            ) : logs.length === 0 ? (
              <div className="grid place-items-center rounded-2xl border border-dashed border-gray-200 py-20">
                <p className="text-sm text-gray-400">No sync activity yet.</p>
              </div>
            ) : filtered.length === 0 ? (
              <div className="grid place-items-center rounded-2xl border border-dashed border-gray-200 py-20">
                <p className="text-sm text-gray-400">No logs match these filters.</p>
              </div>
            ) : (
              <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                {filtered.map((log) => (
                  <LogRow key={log.id} log={log} onOpen={setSelected} />
                ))}
              </div>
            )}
          </>
        )}

{tab === "audit" && (
          <>
            {loading ? (
              <div className="grid place-items-center py-20">
                <p className="text-sm text-gray-400">Loading…</p>
              </div>
            ) : auditTotalCount === 0 ? (
              <div className="grid place-items-center rounded-2xl border border-dashed border-gray-200 py-20">
                <p className="text-sm text-gray-400">No audit history yet.</p>
              </div>
            ) : (
              <>
                <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                  {auditEntries.map((entry) => (
                    <AuditRow key={entry.id} entry={entry} />
                  ))}
                </div>

                <div className="mt-4 flex items-center justify-between">
                  <p className="text-xs text-gray-500">
                    Page {auditPage} of {auditTotalPages} · {auditTotalCount} entr{auditTotalCount === 1 ? "y" : "ies"}
                  </p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setAuditPage((p) => Math.max(1, p - 1))}
                      disabled={auditPage <= 1}
                      className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-[#0b1830] disabled:opacity-40"
                    >
                      Previous
                    </button>
                    <button
                      onClick={() => setAuditPage((p) => Math.min(auditTotalPages, p + 1))}
                      disabled={auditPage >= auditTotalPages}
                      className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-[#0b1830] disabled:opacity-40"
                    >
                      Next
                    </button>
                  </div>
                </div>
              </>
            )}
          </>
        )}
      </div>

      <LogDrawer log={selected} onClose={() => setSelected(null)} onRetry={handleRetry} retrying={retrying} />

      <Toast toasts={toasts} removeToast={removeToast} />
    </main>
  );
}